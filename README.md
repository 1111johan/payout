...
- “申请所有可用站点”会先刷新 11 个站点；US/UK/CA 读取 Seller Central Standard Orders 余额，其他站点查询默认银行账户。确认后先提交紫鸟站点，再按 Amazon 限流间隔依次提交 Transfers API 站点。
...
US/UK/CA 网页付款控制需要与紫鸟内核大版本匹配的 ChromeDriver 和 Selenium。控制台可通过“批量处理 US/UK/CA”自动启动已授权的对应紫鸟店铺，逐站读取标准订单可用资金，并只为当前允许请求付款的站点进入确认页。提交前会集中显示各站金额和银行尾号，确认后逐站提交；任何结果未知的请求都禁止自动重试。未出现在紫鸟自动化账号中的站点不会执行，需先在紫鸟开放平台授权给当前自动化成员。
